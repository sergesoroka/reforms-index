...
import CheckBoxP from "./CheckBoxP";
...
export default function NestedCheckBox({
    ...
    const [checked, setChecked] = useState(
        !!values.some(value => value.includes(label))
    );
    const [childData, setChildData] = useState([]);
    const [nestedValues, setNestedValues]=useState(dataSet.map((item) =>(item.status=='has')?item.date:'').filter(Boolean))
    ...
    const toggleNest=(()=>{
        if (!checked){
            ...
            setValues(prevArray=> prevArray.filter(item => !item.includes(label)))
        }
        setChecked(!checked)
